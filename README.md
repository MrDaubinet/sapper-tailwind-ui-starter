...
Three changes have been made to allow for Tailwind to be included in the sapper project. 

1. The ```tailwindcss``` npm module has been included, along with the ```@tailwindcss/aspect-ratio```, ```@tailwindcss/forms```, and ```@tailwindcss/typography``` modules (these are optional)
2. The package file has been changed to generate the tailwind files with the ```predev```, ```prebuild``` and ```build:tailwind``` commands. 
3. The ```client.ts``` and ```server.ts``` files have the following import included:<br>
  ```import "./tailwind.css";```<br>
which simply includes the compiled tailwind class. 

The tailwind css generation is only executed through the ```prebuild``` or ```predev``` commands (which run before a build or dev npm command). Therefore changing the used tailwind css classes and saving your files (recompiling svelte) will not result in a new generation of this file. 


## Testing
Add any tailwind classes to any svelte file elements.

## Limitations
```@apply``` does not work. I think one would have to edit the svelte preprocess flag in rollup, but preprocessing tailwind everytime you save a file is going to make development slow. 